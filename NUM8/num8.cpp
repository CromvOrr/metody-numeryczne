...
{
    return (abcd[0] * pow(x, 2)) + (abcd[1] * sin(x)) + (abcd[2] * cos(5 * x)) + (abcd[3] * exp(-x));
}
...
{
    return (abcd[0] * sin(x)) + (abcd[1] * cos(x)) + (abcd[2] * x) + (abcd[3] * pow(x, 2));
}
...
VectorXd SVD(MatrixXd &matrix, VectorXd &yValues)
{
    ...
    VectorXd abcd = svd.matrixV() * EInverse.asDiagonal() * svd.matrixU().adjoint() * yValues;
    return abcd;
}
...
int main()
{
    ...
    double aabcd[4] = {eeigenABCD(0), eeigenABCD(1), eeigenABCD(2), eeigenABCD(3)};
