...
void setMarixAandVectorB(int sizeA, vector<vector<int>> &matrixA, vector<int> &b)
{
...
void breakMatrixA(int sizeA, vector<vector<int>> &smallMatrixA, vector<int> &u, vector<vector<int>> &vT)
{
...
void backSubstitution(int sizeA, vector<vector<int>> &smallMatrixA, vector<double> &toBeCalculated, vector<int> &knownVector)
{
    toBeCalculated.resize(sizeA, 0.0);

    toBeCalculated[sizeA - 1] = static_cast<double>(knownVector[sizeA - 1]) / static_cast<double>(smallMatrixA[1][sizeA - 1]);
    for (int i = sizeA - 2; i >= 0; i--)
    {
        toBeCalculated[i] = (static_cast<double>(knownVector[i]) - (static_cast<double>(smallMatrixA[0][i + 1]) * toBeCalculated[i + 1])) / static_cast<double>(smallMatrixA[1][i]);
    }
}

void substituteIntoShermanMorrisonFormula(int sizeA, vector<double> &y, vector<double> &q, vector<vector<int>> &vT, vector<double> &z)
{
