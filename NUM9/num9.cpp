...
double h(double x)
{
    return ((sin(x) - 0.4)) / (2 * cos(x));
}

double dh(double x)
{
    return 0.5 * ((((sin(x) - 0.4) * tan(x)) / cos(x)) + 1);
}
...
void falsi()
{
    ...
    while (true)
    {
        double c = ((-b * function(a)) + (a * function(b))) / (function(b) - function(a));
        results << i++ << "\t" << abs(c - exactRoot) << endl;
...
void siecznych()
{
    ...
    while (true)
    {
        double xiPlus1 = ((xi * function(xiMinus1)) - (xiMinus1 * function(xi))) / (function(xiMinus1) - function(xi));
